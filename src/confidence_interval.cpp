...
int computeIC_bootstraps(InputOutputStream *io, Pr* pr,Node** nodes,double* &T_left,double* &T_right,double* &H_left,double* &H_right,double* &HD_left,double* &HD_right,double &rho_left,double& rho_right,double* &other_rhos_left,double* &other_rhos_right,int r,bool diffTopology){
  ...
  for (int y = 0; y< pr->nbBootstrap; y++){
    ...
    e = tree2data(*(io->inBootstrapTree),pr_bootstrap,s, nodes_bootstrap);
    ...
    if (pr->c == -1){
      double mbl;
      e = median_branch_lengths(pr_bootstrap,nodes_bootstrap,mbl);
      if (e==EXIT_FAILURE) return e;
      pr_bootstrap->b  = max(mbl,10./pr->seqLength);
    } else {
    ...
    if (pr->verbose){
      cout<<"Tree "<<y+1<<" rate: "<<pr_bootstrap->rho<<", rMRCA: "<<nodes_bootstrap[0]->D<<endl;
    }
    if (!diffTopology){
...
int output(double br,int y, InputOutputStream *io, Pr* pr,Node** nodes,ostream& f,ostream& tree2,ostream& tree3,int r,bool diffTopology){
  ...
  if (!pr->ci){
    ...
    if (pr->bootstraps_file==""){
      ...
      int e = computeIC_bootstraps(io,pr,nodes,T_min,T_max,H_min,H_max,HD_min,HD_max,rho_left,rho_right,other_rhos_left,other_rhos_right,r,diffTopology);
      if (e==EXIT_FAILURE) return e;
    }
    std::ostringstream oss;
