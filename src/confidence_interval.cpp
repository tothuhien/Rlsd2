...
int computeIC_bootstraps(InputOutputStream *io, Pr* pr,Node** nodes,double* &T_left,double* &T_right,double* &H_left,double* &H_right,double* &HD_left,double* &HD_right,double &rho_left,double& rho_right,double* &other_rhos_left,double* &other_rhos_right,int r){
  ...
  for (int y = 0; y< pr->nbBootstrap; y++){
    ...
    e = tree2data(*(io->inBootstrapTree),pr,s,nodes_bootstrap);
    ...
    if (pr->c == -1){
       double mbl;
       e = median_branch_lengths(pr,nodes_bootstrap,mbl);
       if (e==EXIT_FAILURE) return e;
       pr->b = max(mbl,10./pr->seqLength);
    } else {
      pr->b = pr->c;
    }
...
int output(double br,int y, InputOutputStream *io, Pr* pr,Node** nodes,ostream& f,ostream& tree1,ostream& tree2,ostream& tree3,int r){
