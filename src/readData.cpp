...
int tree2data(istream& tree,Pr* pr,int & s,Node** &nodes){
  ...
  stack<int> pileNode;
  char c;
  e= readBracket(tree,"input tree",c);
  ...
  do{
    ...
    if (c==')'){
      ...
      nbChild=0;
      s=1;
      Node* new_inode = new Node();
      e = readSupport(tree,"input tree",new_inode->L);
      if (e==EXIT_FAILURE) return e;
      //stack<int> listSuc;
      ...
      if (a>0) {
        e=readdouble(tree,"input tree",new_inode->B);
        if (e==EXIT_FAILURE) return e;
      }
      pileNode.push(inode);
      inode++;
      ...
      string lb;
      e=readLabel(c,tree,a,lb);
      if (e==EXIT_FAILURE) return e;
      pileNode.push(-countleaf);
      Node* new_leaf = new Node();
      new_leaf->L=lb;
      e=readdouble(tree,"input tree",new_leaf->B);
      if (e==EXIT_FAILURE) return e;
  ...
  if (!pr->rooted){
    if (pr->estimate_root=="" && pr->fnOutgroup=="") {
      cerr<<"The input trees are not rooted, use either option outGroup to specify the outgroups file or option estimateRoot to estimate the root"<<endl;
      return (EXIT_FAILURE);
    }
  }
...
int readInputDate(InputOutputStream* io, Pr* pr,Node** &nodes,bool& constraintConsistent){
  ...
  if (io->inDate){
    ...
    int ino;
    e=readInt(*(io->inDate),"Error in the date file, the file should begin with an integer (the number of temporal constrains to read)",ino);
    if (e == EXIT_FAILURE) return e;
    if (lineNb-1<ino) {
      cerr<<"The number of given constraints is smaller than the number of constraints to read. Please change the number of constraints to read at the first line of the input date file."<<endl;
      exit(EXIT_FAILURE);
    }
    for (int i=0;i<ino;i++){
      string s;
      e=readWord(*(io->inDate),pr->inDateFile,s);
      if (e == EXIT_FAILURE) return e;
      int k;
      ...
      if (k==-1 && (s.compare("mrca")==0 || s.compare("ancestor")==0)){
        ...
        while (c!=')'){
          string ss="";
          e=readCommaBracket(*(io->inDate),pr->inDateFile,ss,c);
          if (e == EXIT_FAILURE) return e;
          if (ss==""){
            myExit("Empty node label at line ",i+2,"in the date file.\n");
          }
          int k1;
          e=getPosition(nodes,ss,0,pr->nbBranches+1,k1);
          if (e == EXIT_FAILURE) return e;
          if (k1!=-1){
            mr.push_back(k1);
            if (ld=="") ld=ld+ss;
      ...
      if (k!=-1){
        ...
        e=readWholeDate(*(io->inDate),pr,type,v1,v2,m1,m2,d1,d2,dateFormat);
        if (e == EXIT_FAILURE) return e;
        kSave.push_back(k);
        m1Save.push_back(m1);
        m2Save.push_back(m2);
        ...
        if (i<ino-1){
          char c;
          e=readChar(*(io->inDate),pr->inDateFile,c);
          if (e == EXIT_FAILURE) return e;
          while (c!='\n'){ 
            e=readChar(*(io->inDate),pr->inDateFile,c);
            if (e == EXIT_FAILURE) return e;
  ...
  if (pr->MRCA != ""){
    ...
    e =readWholeDate(*mrca,pr,type,v1,v2,m1,m2,d1,d2,dateFormat);
    if (e == EXIT_FAILURE) return e;
    kSave.push_back(0);
    m1Save.push_back(m1);
    m2Save.push_back(m2);
  ...
  if (pr->LEAVES != ""){
    ...
    e=readWholeDate(*leaves,pr,type,v1,v2,m1,m2,d1,d2,dateFormat);
    if (e == EXIT_FAILURE) return e;
    for (int i=pr->nbINodes; i<=pr->nbBranches;i++){
      tipHaveTime[i] = true;
...
int getBranchOut(Pr* pr,Node** nodes,list<string> &outgroups,bool &keepBelow,int &r){
  ...
  if (outgroups.size()==1) {
    keepBelow=false;
    e = getPosition(nodes,*iter,0,pr->nbBranches+1,r);
    if (e==EXIT_FAILURE) return e;
  }
  else{
    vector<int> out;
...
int extrait_outgroup(InputOutputStream *io, Pr* pr, bool useBootstrapTree){
  ...
  for (int y=0;y<nbData;y++){
    ...
    bool keepBelow;
    int r;
    e=getBranchOut(pr, nodes, outgroups,keepBelow,r);
    if (e==EXIT_FAILURE) return e;
    int nbTips = 0;
    if (r!=-1) {
