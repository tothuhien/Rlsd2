...
int monthDayToReal(int m,int d,double& r){
    ...
    {
        case 1:
            if (d>=1 && d<=31) {r = (double)d/365.; return EXIT_SUCCESS;}
            break;
        case 2:
            if (d>=1 && d<=29) {r = (d+31.)/365.;return EXIT_SUCCESS;}
            break;
        case 3:
            if (d>=1 && d<=31) {r = (d+59.)/365.;return EXIT_SUCCESS;}
            break;
        case 4:
            if (d>=1 && d<=30) {r = (d+90.)/365.;return EXIT_SUCCESS;}
            break;
        case 5:
            if (d>=1 && d<=31) {r = (d+120.)/365.;return EXIT_SUCCESS;}
            break;
        case 6:
            if (d>=1 && d<=30) {r = (d+151.)/365.;return EXIT_SUCCESS;}
            break;
        case 7:
            if (d>=1 && d<=31) {r = (d+181.)/365.;return EXIT_SUCCESS;}
            break;
        case 8:
            if (d>=1 && d<=31) {r = (d+212.)/365.;return EXIT_SUCCESS;}
            break;
        case 9:
            if (d>=1 && d<=30) {r = (d+243.)/365.;return EXIT_SUCCESS;}
            break;
        case 10:
            if (d>=1 && d<=31) {r = (d+273.)/365.;return EXIT_SUCCESS;}
            break;
        case 11:
            if (d>=1 && d<=30) {r = (d+304.)/365.;return EXIT_SUCCESS;}
            break;
        case 12:
            if (d>=1 && d<=31) {r = (d+334.)/365.;return EXIT_SUCCESS;}
            break;
    }
...
int maxDate(int m,int & md){
    ...
    {
    case 1:
        md = 31; return EXIT_SUCCESS;
        break;
    case 2:
        md = 28; return EXIT_SUCCESS;
        break;
    case 3:
        md = 31; return EXIT_SUCCESS;
        break;
    case 4:
        md = 30; return EXIT_SUCCESS;
        break;
    case 5:
        md = 31; return EXIT_SUCCESS;
        break;
...
bool readDateFromString(const char* str,double& f){
    ...
    if (*str=='-'){
        ...
        if(*str=='-'){
            ...
            double r;
            int e = monthDayToReal(atoi(m.c_str()),atoi(d.c_str()),r);
            if (e==EXIT_FAILURE) return e;
            f = atoi(y.c_str())+r;
            return true;
        }
...
int readInt(istream& f,string msg,int& c){
    if (f >> c) {
        c = (int)(c);
        return EXIT_SUCCESS;
    } else {
...
int readSupport(istream& f,string fn,string& s){
    s="";
    char c;
    int e=readChar(f,fn,c);
    if (e==EXIT_FAILURE) return e;
    while (c!=':' && c!=';') {
        s=s+c;
        e=readChar(f,fn,c);
...
int getPosition(Node** nodes,string s,int n,int m,int& k){
    int i = n;
    int count=0;
    while (i<m && count<2){
...
int readLabel(char ch,istream& f,int& a,string& s){
    s="";
    s=s+ch;
    char c;
    int e=readChar(f,"input tree",c);
    if (e==EXIT_FAILURE) return e;
    while (c!=':' && c!=';' && c!=')'){
        s=s+c;
        f.get(c);
...
int readBracket(istream& f,string fn,char& c){
    int e=readChar(f,fn,c);
    if (e==EXIT_FAILURE) return e;
    while (c!='('){
        e=readChar(f,fn,c);
...
int readCommaBracket(istream& f,string fn,string& s,char& c){
    int e =readChar(f,fn,c);
    if (e==EXIT_FAILURE) return e;
    s="";
    while (c==' ' || c=='	'){ 
...
int getOutgroup(istream &o, string fn,list<string>& result){
    int lineNb=getLineNumber(o);
    int ino;
    int e =readInt(o,"Error in the outgroup file, the file should begin with an integer (the number of outgroups)",ino);
    if (e == EXIT_FAILURE) return e;
    if (lineNb-1<ino) {
...
int checkRooted(Pr* &opt){
    ...
    stack<int> pileNode;
    char c;
    int e = readBracket(tree,"input tree",c);
    if (e==EXIT_FAILURE) return e;
    int a=1;
    int s=0;
    int nbChild=0;
    ...
    do{
        ...
        if (c==')'){
            ...
            string sp;
            e = readSupport(tree,"input tree",sp);
            if (e==EXIT_FAILURE) return e;
            if (a>0) {
                double d;
                e = readdouble(tree,"input tree",d);
                if (e==EXIT_FAILURE) return e;
            }
            pileNode.push(1);
            n++;
...
int getInternalNodeId(Pr* pr,Node** nodes,string s,int& k){
    int e = getPosition(nodes,s,0,pr->nbBranches+1,k);
    if (e==EXIT_FAILURE) return e;
    if (k==-1 && (s.substr(0,4).compare("mrca")==0)){
        ...
        while (c!=')'){
            ...
            p = newp+1;
            int k1;
            e=getPosition(nodes,s1,0,pr->nbBranches+1,k1);
            if (e==EXIT_FAILURE) return e;
            if (k1!=-1){
                mr.push_back(k1);
            }
...
int assignRateGroupToSubTree(Subtree* subtree,Pr* pr,Node** nodes,int g,int& s){
    Pair* root = subtree->root;
    int r;
    int e = getInternalNodeId(pr,nodes,root->name,r);
    if (e==EXIT_FAILURE) return e;
    bool toReroot = false;
    if (r >= pr->nbINodes) {
    ...
    for (int i=0; i<subtree->tips.size(); i++) {
        ...
        int t; 
        e = getInternalNodeId(pr,nodes,tip->name,t);
        if (e==EXIT_FAILURE) return e;
        nodes[t]->rateGroup = g;
        s++;
        tipsId.push_back(t);
...
int assignRateGroupToTree(Pr* pr,Node** nodes){
    vector<int> subroot;
    int e;
    ...
    for (int i=0; i<pr->ratePartition.size(); i++) {
        ...
        for (int j=0; j<group->subtrees.size(); j++) {
            ...
            int s;
            e = assignRateGroupToSubTree(subtree,pr,nodes,i+1,s);
            if (e==EXIT_FAILURE) return e;
            nbBranchesPartition += s;
        }
    }
    for (int i=0; i<pr->ratePartition.size(); i++) {
        Part* group = pr->ratePartition[i];
...
void adjustNodeDateToYMD(Node*& node,int m1,int d1,int m2,int d2){
    ...
    if (node->type == 'p'){
        ...
        if (m1<0){//only year
            node->type = 'b';
            monthDayToReal(1,1,r);
            node->lower = sign*(year + r);
            monthDayToReal(12,31,r);
            node->upper = sign*(year + r);
        } else if (d1<0){//only year-month
            node->type = 'b';
            monthDayToReal(m1,1,r);
            node->lower = sign*(year + r);
            maxDate(m1,md);
            monthDayToReal(m1,md,r);
            node->upper = sign*(year + r);
        }
    }
    else if (node->type == 'l'){
...
void adjustDateToYMD(Date*& date,int m1,int d1,int m2,int d2){
    ...
    if (date->type == 'p'){
        ...
        if (m1<0){//only year
            date->type = 'b';
            monthDayToReal(1,1,r);
            date->lower = sign*(year + r);
            monthDayToReal(12,31,r);
            date->upper = sign*(year + r);
        } else if (d1<0){//only year-month
            date->type = 'b';
            monthDayToReal(m1,1,r);
            date->lower = sign*(year + r);
